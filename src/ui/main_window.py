...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.timeout_spin.setRange(5, 999999)
    ...
    def start_translation(self):
        try:
            if not self.init_processor_and_chunks():
                return
            ...
            # 检测是否已经全部翻译完成（current_flat_idx 已到末尾）
            if self.current_cache_data:
                total_chunks = sum(len(f.get("chunks", [])) for f in self.current_cache_data.get("files", []))
                current_idx = self.current_cache_data.get("current_flat_idx", 0)
                if current_idx >= total_chunks and total_chunks > 0:
                    reply = QMessageBox.question(
                        self, "翻译已完成",
                        f"所有 {total_chunks} 个分块均已翻译完成（进度 {current_idx}/{total_chunks}）。\n\n"
                        "如需重新翻译，请：\n"
                        "• 选中部分行后点击「翻译选中组」重新翻译指定块\n"
                        "• 或点击「清除缓存」后重新开始\n\n"
                        "是否直接导出翻译结果？",
    ...
    def on_progress(self, current_idx, total, orig, trans, is_finished, error_type="ok"):
        # 严格非流式：忽略所有中间状态
        if not is_finished:
            if error_type == "starting":
                self.status_label.setText(f"正在启动翻译任务: {current_idx + 1}/{total} (等待 interval 间隔...)")
            else:
